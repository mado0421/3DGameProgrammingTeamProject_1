...
void CPlayer::Move(DWORD dwDirection, float fDistance, bool bUpdateVelocity)
{
	if (dwDirection)
	{
		...
		m_xmf3MovingDirection = xmf3Shift;
...
CTerrainPlayer::CTerrainPlayer(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList
	*pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, void *pContext) : CPlayer()
{
	...
	*/
	...
	SetPosition(XMFLOAT3(pTerrain->GetWidth()*0.2f+200, fHeight + 1500.0f, pTerrain->GetLength()*0.2f+200));
	...
	SetOOBB(XMFLOAT3(0, 0, 0), XMFLOAT3(2, 6, 2), XMFLOAT4(0, 0, 0, 1));
}
...
void CTerrainPlayer::OnCameraUpdateCallback(float fTimeElapsed)
{
	...
	이렇게 되면 다음 그림의 왼쪽과 같이 지형이 그려지지 않는 경우가 발생한다
	(카메라가 지형 안에 있으므로 삼각형의 와인딩 순서가 바뀐다).

	이러한 경우가 발생하지 않도록 카메라의 위치 벡터의 y-값의 최소값은 (지형의 높이 + 5)로 설정한다.

	카메라의 위치 벡터의 y-값의 최소값은 지형의 모든 위치에서카메라가 지형 아래에 위치하지 않도록 설정해야 한다.
	*/
