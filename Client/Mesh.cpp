...
높이 맵을 사용하여 지형을 표현할 수 있는 격자(바둑판과 같은) 메쉬를 정의하자.

CHeightMapGridMesh 클래스는 높이 맵을 사용하는 하나의 격자 메쉬를 표현한다.

이 격자는 좌표계의 xz-평면 위에 있다고 가정한다.

그리고 격자의 간격은 1이 되도록 만들 것이다
(실제 지형에서 격자의 간격은 스케일 벡터에 따라달라질 것이다).
그러면 격자의 각 교차점은 메쉬의 정점이 될 것이다

이제 이 격자의 각 교차점의 y-좌표(높이)는 교차점의 x-좌표가 a,
z-좌표가 b일 때 높이 맵(이미지)에서 (a, b) 좌표의 픽셀 값을 사용한다.
	...
	nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void *pContext) : CMesh(pd3dDevice,pd3dCommandList)
{
	...
	m_nStride = sizeof(CIlluminated2TexturedVertex);
	//격자는 삼각형 스트립으로 구성한다.
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP;
	m_nWidth = nWidth;
	m_nLength = nLength;
	m_xmf3Scale = xmf3Scale;
	CIlluminated2TexturedVertex *pVertices = new CIlluminated2TexturedVertex[m_nVertices];
	...
	for (int i = 0, z = zStart; z < (zStart + nLength); z++)
	{
		for (int x = xStart; x < (xStart + nWidth); x++, i++)
		{
			...
			pVertices[i].m_xmf3Position = XMFLOAT3(xmf3Position.x, xmf3Position.y, xmf3Position.z);
			pVertices[i].m_xfm3Color = Vector4::Add(OnGetColor(x, z, pContext), xmf4Color);
			pVertices[i].m_xmf3Normal = reinterpret_cast<CHeightMapImage*>(pContext)->GetHeightMapNormal(x,z);
...
XMFLOAT4 CHeightMapGridMesh::OnGetColor(int x, int z, void *pContext)
{
	...
	의 정점 (x+1, z), (x, z+1), (x+1, z+1)의 법선 벡터와 조명의 방향 벡터의 내적을 평균하여 구한다.
...
CPlaneMeshTextured::CPlaneMeshTextured(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, float fWidth, float fHeight) :CMesh(pd3dDevice,pd3dCommandList)
{
	m_nVertices = 6;
	...
	int i = 0;
	pxmf3Positions[i++] = XMFLOAT3(-fx, fy, 0);	//0
	pxmf3Positions[i++] = XMFLOAT3(+fx, fy, 0);	//1
	pxmf3Positions[i++] = XMFLOAT3(-fx, -fy, 0);//2

	pxmf3Positions[i++] = XMFLOAT3(-fx, -fy, 0);//2
	pxmf3Positions[i++] = XMFLOAT3(+fx, fy, 0);//1
	pxmf3Positions[i++] = XMFLOAT3(+fx, -fy, 0);//3

	XMFLOAT2 pxmf2TexCoords[6];
	i = 0;
	pxmf2TexCoords[i++] = XMFLOAT2(0.0f, 0.0f);//0
	pxmf2TexCoords[i++] = XMFLOAT2(1.0f, 0.0f);//1
	pxmf2TexCoords[i++] = XMFLOAT2(0.0f, 1.0f);//2

	pxmf2TexCoords[i++] = XMFLOAT2(0.0f, 1.0f);//2
	pxmf2TexCoords[i++] = XMFLOAT2(1.0f, 0.0f);//1
	pxmf2TexCoords[i++] = XMFLOAT2(1.0f, 1.0f);//3


	CTexturedVertex pVertices[6];
	for (int i = 0; i < 6; i++) pVertices[i] = CTexturedVertex(pxmf3Positions[i], pxmf2TexCoords[i]);
...
CmmeMesh::CmmeMesh(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList):CMesh(pd3dDevice, pd3dCommandList)
{
	FILE* pFile=NULL;
	pFile = fopen("model_new.mme", "rb");
	
	//assert(pFile&&"CmmeMesh Model Read Error");

	LONG numOfElement;
	fread(&numOfElement, sizeof(LONG), 1, pFile);

	CmmeVertex* Vertex = new CmmeVertex[numOfElement];
	fread(Vertex, sizeof(CmmeVertex), numOfElement, pFile);
	fclose(pFile);

	m_nVertices = numOfElement;
	m_nStride = sizeof(CmmeVertex);
	m_nOffset = 0;
	m_nSlot = 0;
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;


	m_pd3dVertexBuffer = CreateBufferResource(pd3dDevice, pd3dCommandList, Vertex, m_nStride * m_nVertices, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dVertexUploadBuffer);
