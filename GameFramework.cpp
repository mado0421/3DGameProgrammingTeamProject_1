...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case 'N':
			...
			break;
		case 'D':
			pfClearColor[0] = 243.0 / 255;
			pfClearColor[1] = 80.0 / 255;
			pfClearColor[2] = 56.0/255;
			pfClearColor[3] = 1.0f;
			break;
		}
		case WM_KEYUP:
			switch (wParam)
			{
				...
				//case VK_F3:
					...
					if (m_pScene)m_pScene->SetCameraContext(m_pCamera);
					break;
				case VK_F9:
				{
...
void CGameFramework::BuildObjects()
{
	...
	//CmmeMesh* me = new CmmeMesh(m_pd3dDevice, m_pd3dCommandList);
	WaitForGpuComplete();
		...
		if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f))
		{
			if (cxDelta || cyDelta)
			{
				if (pKeysBuffer[VK_RBUTTON] & 0xF0)
					m_pPlayer->Rotate(cyDelta/2, 0.0f, -cxDelta/2);
				else
					m_pPlayer->Rotate(cyDelta/2, cxDelta/2, 0.0f);
			}
